...
void ControlPlaneClient::register_node(
    ...
    if (required_string(data, "node_code") != node.node_code) {
        throw ControlPlaneClientError{
            ControlPlaneErrorKind::InvalidResponse,
...
void ControlPlaneClient::send_heartbeat(
    ...
    const auto data = success_data(response.status, response.body);
    if (required_string(data, "node_code") != heartbeat.node_code) {
        throw ControlPlaneClientError{
            ControlPlaneErrorKind::InvalidResponse,
...
PulledAgentConfig ControlPlaneClient::fetch_config(
    ...
    result.node.node_code = required_string(node, "node_code");
    result.node.name = required_string(node, "name");
    result.node.agent_version = required_string(node, "agent_version");
    result.last_heartbeat_at = required_string(node, "last_heartbeat_at");
    const auto status = required_string(node, "status");
    if (status == "online") {
    ...
    for (const auto& task_value : data["tasks"]) {
        ...
        task.id = required_string(task_value, "id");
        task.node_code = required_string(task_value, "node_code");
        task.data_source_id = required_string(task_value, "data_source_id");
        task.name = required_string(task_value, "name");
        task.task_type = required_string(task_value, "task_type");
        task.schedule_expr = required_string(task_value, "schedule_expr");
        task.parser_type = required_string(task_value, "parser_type");
        task.qc_profile = required_string(task_value, "qc_profile");
        task.enabled = required_boolean(task_value, "enabled");
