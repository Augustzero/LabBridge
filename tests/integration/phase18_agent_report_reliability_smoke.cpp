...
int main() {
    {
        ...
        assert((session.commands == std::vector<std::string>{"BEGIN", "ROLLBACK"}));
    }
    {
        RecordingSqlSession session;
        {
        ...
        assert((session.commands == std::vector<std::string>{"BEGIN", "COMMIT"}));
    }
    ...
    assert(!agent_report_service.accept_raw_file_manifest(missing_key).status.ok);
    ...
    assert(manifest_replay.raw_file_ids == manifest.raw_file_ids);
    assert(result_service.find_raw_files(started.id).size() == 1);
    ...
    assert(!conflicting_manifest.status.ok);
    assert(conflicting_manifest.status.code == labbridge::core::StatusCode::Conflict);
    ...
    assert(result_service.find_parsed_records(started.id).size() == 1);
    ...
    assert(!conflicting_report.status.ok);
    assert(conflicting_report.status.code == labbridge::core::StatusCode::Conflict);
    ...
    assert(!second_report_result.status.ok);
    assert(second_report_result.status.code == labbridge::core::StatusCode::Conflict);
